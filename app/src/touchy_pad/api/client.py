...
class TouchyClient:
    ...
    def set_property(
        ...
        * :class:`~touchy_pad.api.Color` → ``0xRRGGBB`` colour property,
        * :class:`~touchy_pad.api.Point` → ``(x, y)`` point property,
        * ``None`` → **remove** any existing override for this
          ``(widget_id, prop)``.

        The override is session-scoped (RAM only) and sticky: the device
        re-applies it whenever the widget is (re)built and immediately if
        it is already on screen. Targeting a not-yet-loaded widget is fine.
        """
        ...
        if value is None:
            ...
            raise TypeError(
                "value must be bool / int / str / Color / Point / None, "
                f"got {type(value).__name__}"
            )
        _check(self._rpc(_proto.Command(set_property=cmd)))
