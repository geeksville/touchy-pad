...
//
// Board bring-up for the Waveshare ESP32-S3-Matrix LED-matrix board (Stage
// matrix1). Display-less / touch-less: there is no shared I2C bus and no
// touch controller. The LED panel itself is created by display_init();
// board_init only handles the (currently minimal) board-wide setup.
...
{
    ESP_LOGI(TAG, "Board init done (ESP32-S3-Matrix, 8x8 LED matrix)");
}
...
// Stage matrix1 — strong override of the weak platform_is_touchable() default:
