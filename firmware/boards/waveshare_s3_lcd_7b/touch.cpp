...
extern "C" esp_lcd_touch_handle_t touch_init(lv_disp_t *disp)
{
    ...
    // Create the I2C panel-io device for the touch controller.
    // NB: We can't use ESP_LCD_TOUCH_IO_I2C_GT911_CONFIG() from the managed
    // component – its designator order no longer matches IDF 6.0.1's struct
    // layout (which is a hard error in C++).
    ...
    tp_cfg.rst_gpio_num = BOARD_TOUCH_RST_GPIO;     // RST handled via CH422G in board_init()
    ...
    // Hand off to LVGL so the GUI side gets touch events too.
