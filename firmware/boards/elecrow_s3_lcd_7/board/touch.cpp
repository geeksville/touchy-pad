...
// Try to find the GT911 at its primary address; fall back to the secondary.
...
// Create esp_lcd_panel_io + esp_lcd_touch handle for GT911.
// Returns the touch handle on success, nullptr on failure.
static esp_lcd_touch_handle_t create_gt911(void)
{
    ...
    io_cfg.scl_speed_hz         = c->i2c_clk_hz;
    ...
    tp_cfg.rst_gpio_num = GPIO_NUM_NC;  // reset handled by PCA9557 in board_init()
    tp_cfg.int_gpio_num = c->touch_int_gpio;
